...
def _heartbeat_loop(window, api=None):
    ...
    global _hb_started
    try:
        _empty_dat = 0          # 连续读不到激活码（激活后 dat 被删/改名 = 篡改特征）
        _unreachable_since = 0  # 连续联系不上服务端的起点（长断网降级锁定）
        while True:
            ...
            try:
                ...
                if not code:
                    ...
                    _empty_dat += 1
                    if _empty_dat >= 2:
                        _reject(msg="激活信息丢失，请重新激活", api=api, window=window)
                        return
                    continue
                _empty_dat = 0
                state, msg, announcement = lic.heartbeat(code)
                if state == "rejected":
                    # 服务端明确拒绝（吊销/过期/版本报废/设备不匹配）：停网关锁前端
                    _reject(msg=msg, api=api, window=window)
                    return
                if state == "unreachable":
                    # 断网不惩罚是可用性取舍，但网关连跑 24h 联系不上服务端 =
                    # 单方面切断吊销通道（hosts 屏蔽/防火墙），降级锁定
                    now = time.time()
                    if not _unreachable_since:
                        _unreachable_since = now
                    elif now - _unreachable_since > 24 * 3600:
                        _reject(msg="长时间无法连接授权服务器，请检查网络后重新启动", api=api, window=window)
                        return
                    continue
                _unreachable_since = 0
        ...
        # 线程任何退出路径（吊销/删dat/断网24h）都复位标志：重新激活后
        # check_license → start_license_heartbeat 能重新拉起，吊销通道不失联
        _hb_started = False


def _reject(msg, api, window):
    """授权明确失效：停网关 → 锁前端。只锁界面的网关还在跑，授权就形同虚设；
    正在服务的请求由 uvicorn 优雅收尾。"""
    from src.gui.log_setup import write_runtime_log
